pub mod smbios {
    ...
    impl Anchor {
        pub fn new(address: u64, rev: AnchorVer) -> Anchor {
            let anchor_raw_data = match rev {
                AnchorVer::Rev2 => read_raw_mem(address, ANCHOR_REV_2_LENGHT).unwrap(),
                AnchorVer::Rev3 => read_raw_mem(address, ANCHOR_REV_3_LENGHT).unwrap(),
            };
        ...
        fn create_anchor_rev_2 (raw_anchor_data: Vec<u8>) -> Anchor {

        }

        fn create_anchor_rev_3 (raw_anchor_data: Vec<u8>) -> Anchor {

        }
    }

    fn get_smbios_anchor() -> Result<Anchor, u8> {
        for address in SMBIOS_ANCHOR_AREA_START..SMBIOS_ANCHOR_AREA_END - 3 {
            ...
            if let Ok(ascii) = ascii_data {
                if ascii.to_ascii_uppercase() == "_SM".to_ascii_uppercase() {
                    return Ok(Anchor::new(address));
                }
            }
        }
        Err(0)
    ...
    mod tests{
        ...
        fn get_anchor_test () {
            ...
            assert_eq!(good_anchor.entry_point_checksum, 0x01);
        }
    }
